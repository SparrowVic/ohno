...
type PaintTone = 'primary' | 'cool' | 'warm' | 'success' | 'danger' | 'ghost' | 'muted';
...
const GRAPH_RING: readonly GraphNode[] = [
  { id: 'a', x: 42, y: 44 },
  { id: 'b', x: 84, y: 20 },
  { id: 'c', x: 126, y: 44 },
  { id: 'd', x: 172, y: 18 },
...
const GRAPH_RING_EDGES: ReadonlyArray<readonly [string, string]> = [
  ['a', 'b'],
  ['a', 'j'],
  ['b', 'c'],
  ['c', 'd'],
...
const TREE_NODES: readonly GraphNode[] = [
  { id: 'r', x: 160, y: 14 },
  { id: 'a', x: 112, y: 34 },
  { id: 'b', x: 208, y: 34 },
  { id: 'c', x: 80, y: 58 },
...
const TREE_EDGES: ReadonlyArray<readonly [string, string]> = [
  ['r', 'a'],
  ['r', 'b'],
  ['a', 'c'],
  ['a', 'd'],
...
const DAG_NODES: readonly GraphNode[] = [
  { id: 'a', x: 44, y: 20 },
  { id: 'b', x: 44, y: 56 },
  { id: 'c', x: 112, y: 20 },
  { id: 'd', x: 112, y: 56 },
...
const NETWORK_NODES: readonly GraphNode[] = [
  { id: 's', x: 34, y: 38 },
  { id: 'u', x: 108, y: 20 },
  { id: 'v', x: 108, y: 58 },
  { id: 'x', x: 186, y: 20 },
...
const NETWORK_EDGES: ReadonlyArray<readonly [string, string]> = [
  ['s', 'u'],
  ['s', 'v'],
  ['u', 'x'],
  ['u', 'y'],
...
const BIPARTITE_NODES: readonly GraphNode[] = [
  { id: 'l1', x: 78, y: 16 },
  { id: 'l2', x: 78, y: 38 },
  { id: 'l3', x: 78, y: 60 },
  { id: 'r1', x: 242, y: 16 },
...
const BIPARTITE_EDGES: ReadonlyArray<readonly [string, string]> = [
  ['l1', 'r1'],
  ['l1', 'r2'],
  ['l2', 'r2'],
  ['l2', 'r3'],
...
function mergeSpecs(...specs: readonly PreviewSpec[]): PreviewSpec {
  return specs.reduce(
    (acc, spec) => ({
...
function polylinePath(points: ReadonlyArray<readonly [number, number]>, close = false): string {
  if (points.length === 0) {
    return '';
  }
...
function resolvePreviewSpec(algorithm: AlgorithmItem): PreviewSpec {
  const sortingVariant = SORTING_VARIANTS[algorithm.id];
  if (sortingVariant) {
    return buildSortingPreview(sortingVariant);
  }

  const searchVariant = SEARCH_VARIANTS[algorithm.id];
  if (searchVariant) {
    return buildSearchPreview(searchVariant);
  }

  const graphVariant = GRAPH_VARIANTS[algorithm.id];
  if (graphVariant) {
    return buildGraphPreview(graphVariant);
  }

  const dpVariant = DP_VARIANTS[algorithm.id];
  if (dpVariant) {
    return buildDpPreview(dpVariant);
  }

  const stringVariant = STRING_VARIANTS[algorithm.id];
  if (stringVariant) {
    return buildStringPreview(stringVariant);
  }

  const geometryVariant = GEOMETRY_VARIANTS[algorithm.id];
  if (geometryVariant) {
    return buildGeometryPreview(geometryVariant);
  }

  const miscVariant = MISC_VARIANTS[algorithm.id];
  if (miscVariant) {
    return buildMiscPreview(miscVariant);
  }
