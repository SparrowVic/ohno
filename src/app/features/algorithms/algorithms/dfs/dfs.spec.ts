...
describe('dfs', () => {
  it('uses stack order to dive deep before backtracking', () => {
    ...
    expect(finalStep?.graph?.visitOrder).toEqual(['A', 'B', 'D', 'C']);
    expect(dRow?.distance).toBe(2);
    expect(dRow?.secondaryText).toBe('B');
    expect(steps.some((step) => step.phase === 'skip-relax')).toBe(true);
  });

  it('leaves disconnected nodes untouched after the source component is exhausted', () => {
